...
contract StickersReceiver is ERC721, Authentication, IntegrityInterface {
    //has run a sucesful setup call

    //public vars
    StickerOracle public stickerManager;
    ...
    constructor(
        ...
        stickerManager = StickerOracle(oracleDestination);
    ...
    function request(
        ...
        StickerOracle.RegistrationObject memory obj = stickerManager
        ...
        require(
            utility || stickerManager.canAfford(pps, duration, _sender()),
    ...
    function getSticker(
        uint256 _stickerId
    ) external view returns (Sticker memory sticker) {
    ...
    function setup() public onlyDeployer {
        ...
        if (stickerManager.verifyRegistration(tokenId, address(this))) {
            hasSetup = true;
            return;
        }

        //Will throw
        stickerManager.registerDeploymentWithOracle(tokenId, address(this));
        hasSetup = true;
    }
}
