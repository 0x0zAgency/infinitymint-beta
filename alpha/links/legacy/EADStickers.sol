...
contract EADStickers is Stickers, ERC721 {
    ...
    function setProfitWallet(address profitWalletAddress) public onlyDeployer {
        require(isContract(profitWalletAddress), 'is not a contract');
        require(
            Account(payable(profitWalletAddress)).isAuthenticated(_sender()),
    ...
    function updateSticker(uint32 stickerId, bytes memory packed) public {
        require(isApprovedOrOwner(_sender(), uint256(stickerId))); // ERC721 permissions can update the sticker
    ...
    function acceptRequest(
        address sender,
        uint32 index
    ) public override onlyApproved onlyOnce {
        ...
        if (
            ...
            profitWallet.deposit{ value: price }();
        }
    ...
    function addRequest(bytes memory packed) public payable override onlyOnce {
        ...
        require(
            balanceOf(_sender()) < 100,
            'you have minted the maximum amount of stickers to this wallet, use another wallet to mint more.'
        );

        //add it!
        requests[_sender()].push(abi.encode(msg.value, _sender(), packed));
        if (!hasOpenRequests(_sender())) openRequests.push(_sender());

        emit EASRequestAdded(
            uint32(requests[_sender()].length - 1),
            _sender(),
            msg.value,
            packed
        ); //emit
