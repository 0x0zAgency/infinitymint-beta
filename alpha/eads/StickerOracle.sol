...
contract StickerOracle is ERC721, Authentication, InfinityMintObject {
    ...
    function getRegistration(
        uint256 thatOracleId
    ) external view returns (RegistrationObject memory) {
        require(
            registrations[thatOracleId].valid,
            'invalid registration at this id'
        );
        return registrations[thatOracleId];
    }
    ...
    function hasStickerCompleted(
        address stickerDestination,
        uint256 stickerId
    ) external view returns (bool) {
        return
    ...
    function getStickerRegistration(
        uint256 tokenId
    ) external view returns (StickerDestination memory dest) {
    ...
    function checkSenderIsAuthenticated(
        address stickerDestination
    ) private view returns (bool) {
